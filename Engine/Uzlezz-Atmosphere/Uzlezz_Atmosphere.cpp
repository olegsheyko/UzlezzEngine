//***************************************************************************************
// Uzlezz_Atmosphere.cpp by Frank Luna (C) 2015 All Rights Reserved.
//***************************************************************************************
...
class Uzlezz_Atmosphere : public D3DApp
{
public:
    Uzlezz_Atmosphere(HINSTANCE hInstance);
    Uzlezz_Atmosphere(const Uzlezz_Atmosphere& rhs) = delete;
    Uzlezz_Atmosphere& operator=(const Uzlezz_Atmosphere& rhs) = delete;
    ~Uzlezz_Atmosphere();
    ...
    PSTR cmdLine, int showCmd)
{
    ...
    try
    {
        Uzlezz_Atmosphere theApp(hInstance);
...
Uzlezz_Atmosphere::Uzlezz_Atmosphere(HINSTANCE hInstance)
    : D3DApp(hInstance)
{
...
Uzlezz_Atmosphere::~Uzlezz_Atmosphere()
{
    if(md3dDevice != nullptr)
        FlushCommandQueue();
}

bool Uzlezz_Atmosphere::Initialize()
{
...
void Uzlezz_Atmosphere::CreateRtvAndDsvDescriptorHeaps()
{
...
void Uzlezz_Atmosphere::OnResize()
{
    D3DApp::OnResize();
...
void Uzlezz_Atmosphere::Update(const GameTimer& gt)
{
    OnKeyboardInput(gt);
...
void Uzlezz_Atmosphere::Draw(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::OnMouseDown(WPARAM btnState, int x, int y)
{
    mLastMousePos.x = x;
    mLastMousePos.y = y;
...
void Uzlezz_Atmosphere::OnMouseUp(WPARAM btnState, int x, int y)
{
    ReleaseCapture();
}

void Uzlezz_Atmosphere::OnMouseMove(WPARAM btnState, int x, int y)
{
...
{
    // Rotate the skull model around the Y axis.
    if(mSkullRitem != nullptr)
    {
        // rotation speed (radians/sec)
        const float speed = 0.8f;
        mSkullRotationAngle += speed * gt.DeltaTime();

        XMMATRIX S = XMMatrixScaling(0.4f, 0.4f, 0.4f);
        XMMATRIX R = XMMatrixRotationY(mSkullRotationAngle);
        XMMATRIX T = XMMatrixTranslation(0.0f, 1.0f, 0.0f);

        XMMATRIX W = S * R * T; // scale -> rotate -> translate (same as original setup)
        XMStoreFloat4x4(&mSkullRitem->World, W);

        // Mark object dirty so its constant buffer gets updated for all frame resources.
        mSkullRitem->NumFramesDirty = gNumFrameResources;
    }
}

void Uzlezz_Atmosphere::UpdateObjectCBs(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::UpdateMaterialBuffer(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::UpdateShadowTransform(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::UpdateMainPassCB(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::UpdateShadowPassCB(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::UpdateSsaoCB(const GameTimer& gt)
{
...
void Uzlezz_Atmosphere::LoadTextures()
{
...
void Uzlezz_Atmosphere::BuildRootSignature()
{
...
void Uzlezz_Atmosphere::BuildSsaoRootSignature()
{
    CD3DX12_DESCRIPTOR_RANGE texTable0;
...
void Uzlezz_Atmosphere::BuildDescriptorHeaps()
{
	//
	// Create the SRV heap.
	//
...
void Uzlezz_Atmosphere::BuildShadersAndInputLayout()
{
...
void Uzlezz_Atmosphere::BuildShapeGeometry()
{
...
void Uzlezz_Atmosphere::BuildSkullGeometry()
{
...
void Uzlezz_Atmosphere::BuildPSOs()
{
...
void Uzlezz_Atmosphere::BuildFrameResources()
{
...
void Uzlezz_Atmosphere::BuildMaterials()
{
...
void Uzlezz_Atmosphere::BuildRenderItems()
{
    ...
    // keep same initial transform as before: scale 0.4 and place at y=1
    ...
    // Keep a pointer to the skull render item so we can animate it every frame.
...
void Uzlezz_Atmosphere::DrawRenderItems(ID3D12GraphicsCommandList* cmdList, const std::vector<RenderItem*>& ritems)
{
...
void Uzlezz_Atmosphere::DrawSceneToShadowMap()
{
...
void Uzlezz_Atmosphere::DrawNormalsAndDepth()
{
	mCommandList->RSSetViewports(1, &mScreenViewport);
    mCommandList->RSSetScissorRects(1, &mScissorRect);
...
CD3DX12_CPU_DESCRIPTOR_HANDLE Uzlezz_Atmosphere::GetCpuSrv(int index)const
{
...
CD3DX12_GPU_DESCRIPTOR_HANDLE Uzlezz_Atmosphere::GetGpuSrv(int index)const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE Uzlezz_Atmosphere::GetDsv(int index)const
{
...
CD3DX12_CPU_DESCRIPTOR_HANDLE Uzlezz_Atmosphere::GetRtv(int index)const
{
...
std::array<const CD3DX12_STATIC_SAMPLER_DESC, 7> Uzlezz_Atmosphere::GetStaticSamplers()
{
